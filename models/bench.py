...
class DetBenchPredict(nn.Module):
    ...
    def forward(self, x, img_info: Optional[Dict[str, torch.Tensor]] = None):
        class_out, box_out, att_out = self.model(x)
...
class DetBenchTrainImagePair(DetBenchTrain):
    ...
    def forward(self, thermal_img, rgb_img, target: Dict[str, torch.Tensor], eval_pass=False, branch='fusion'):
        class_out, box_out, att_out = self.model([thermal_img, rgb_img], branch=branch)
        ...
        if eval_pass:
            ...
            class_out_pp, box_out_pp, indices, classes = _post_process(
                class_out, box_out, num_levels=self.num_levels, num_classes=self.num_classes,
                max_detection_points=self.max_detection_points)
            assert thermal_img.shape[0] == rgb_img.shape[0], 'Shape mismatch rgb {} vs thermal {}'.format(
                rgb_img.shape[0], thermal_img.shape[0])
            loss, _, _ = self.loss_fn(class_out, box_out, cls_targets, box_targets, num_positives)
            output = {'loss': loss, 'class_out': class_out, 'box_out': box_out, 'detections': _batch_detection(
                thermal_img.shape[0], class_out_pp, box_out_pp, self.anchors.boxes, indices, classes,
                target['img_scale'], target['img_size'],
                max_det_per_image=self.max_det_per_image, soft_nms=self.soft_nms), 'att_out': att_out}
        else:
            loss, _, _ = self.loss_fn(class_out, box_out, cls_targets, box_targets, num_positives)
            output = {'loss': loss, 'class_out': class_out, 'box_out': box_out, 'att_out': att_out}
        return output
...
class DetBenchPredictImagePair(DetBenchPredict):
    ...
    def forward(self, thermal_img, rgb_img, img_info: Optional[Dict[str, torch.Tensor]] = None, branch='fusion'):
        ...
        assert thermal_img.shape[0] == rgb_img.shape[0], 'Shape mismatch rgb {} vs thermal {}'.format(rgb_img.shape[0],
                                                                                                      thermal_img.shape[
                                                                                                          0])
        return _batch_detection(
